...
Kaggle Competition Score : 0.21138
...
**We will fill NA's with `TA` rating.**
...
**Let's try out regression and analyze the results. We tried garage area as well but found that it is not a better predictor than garage cars. Some of the categories for `GarageQual` have been deleted as the result of backward selection for predictors whose p-values were above 0.05 critical values (fails to reject null hypothesis for non-zero slope).**

```{r}
lin_reg<- lm(data=train_df, log(SalePrice) ~ log(GrLivArea)+ TotalBsmtSF + 
               OverallQual + GarageCars +   GarageQualPo + GarageQualGd +
               GarageQualTA)
summary(lin_reg)
```
...
test_df<-test_df %>%
  dplyr::select(Id, GrLivArea, TotalBsmtSF, 
                OverallQual, GarageCars, GarageQual)
