...
The SMS dataset can be bound as well
...
Some classifiers (like XGBoost) need the target class to numerical, others do not (like Navie Bayes). I'll make both so that we can try out different modeling methods. 
...
text_dtm <- removeSparseTerms(text_dtm,sparse = 0.91)
...
2. The Navie Bayes Classifier worked the best, but still needed several testing iterations to get to 91.6% accuracy. Filtering the spare terms was essential and brought the classifier from 52% accuracy to 78%. Second, a little more sparse filtering and TFIDF scoring pulled the classifier up to 91.6% accuarcy. 
...
Awesome. There is some tricky spam in the sms dataset. Without the SMS dataset, this classifier had 97% accuaracy. 
