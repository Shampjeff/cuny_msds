...
I will further scrub the matches to the individual Win, Loss, Draw, etc and the opponent number. The following functions separate and organize those results. Pulling out the game outcome was not required, but I wanted to extract and store in case of further analysis. 
...
make.game.outcome<- function(games_list, game_num){
  ...
  for (i in 1:length(games_list)){
    # This pulls out the win, lose, draw, etc outcome
...
Now for each row I need to replace the opponent number with that opponents pre-tournament ranking. The following function iterates through each column and all the rows and swaps the opponent rank with their respective pre-tournament score. 
...
Once we have the scores in place, we can calculate the average opponent score for each player. The following function makes a new calculated column where is iterate through every row and sums the pre-tournament scores of each opponent divided by the total number of non-NA values, which is the total number of games played per player. 
...
```{r}
df_final<-df[ ,c('name', 'states','points','score', 'opp_average')]
df_final
```
