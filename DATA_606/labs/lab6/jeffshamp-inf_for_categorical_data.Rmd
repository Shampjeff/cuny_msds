...
**These appear to be population parameters. They refer to entire countries in terms of percent religious or not.**
...
**To be able to infer from the sample we need at meaningful sample relative to the population and we need the sample to be collect via a simple random sample technique, the sample must be >30 observations, and the normally distributed. The article doesn't say anything about random sampling or normal distribution.**
...
**In table 6 each row is one country, it's sample, and the precentt agree with each question. Each row in atheism is the response of every person in their respective country.**
...
us12_ath_count<-count(subset(us12, response=="atheist"))
paste("percent atheist in US:",us12_ath_count/dim(us12)[1])
```
...
**The samples neecd to be independent, i.e. simple random sample and we need to see at least 10 successes and 10 failures and be nearly normal.**
...
**ME is Z*sqrt(p*(1-p)/n)**
```{r}
1.96*sqrt((0.499*(1-0.499))/1002)
```
...
```{r}
pl12 <- subset(atheism, nationality == "Palestinian territories (West Bank and Gaza)" & year == "2012")
esp12<- subset(atheism, nationality == "Spain" & year == "2012")
pl12_ath_count<-count(subset(pl12, response=="atheist"))
esp12_ath_count<-count(subset(esp12, response=="atheist"))
paste("percent atheist in Palestine:",pl12_ath_count/dim(pl12)[1], "total responses:", dim(pl12)[1])
paste("percent atheist in Spain:",esp12_ath_count/dim(esp12)[1],"total responses:", dim(esp12)[1])
```

```{r}
inference(pl12$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
```
...
**inference conditions met for Palestine and Spain.**
...
**Quadratic with symmetry at around 0.5, downward facing, and vertex at approx (0.5, 0.03)**
...
**This sampling distribution is normal (as expected) and centered at 0.1, very close to the true atheist proportion and has standard deviation of 0.0092.**
...
p <- c(0.1,0.02, 0.02)
n <- c(400, 1040, 400)
p_hats_j <-list(rep(0, 5000),rep(0, 5000),rep(0, 5000))

for (j in 1:3){
for(i in 1:5000){
  samp <- sample(c("atheist", "non_atheist"), n[j], replace = TRUE, prob = c(p[j], 1-p[j]))
  p_hats_j[[j]][i] <- sum(samp == "atheist")/n[j]
}
}
par(mfrow = c(2,2))
hist(p_hats, main = "p = 0.1, n = 1040", xlim = c(0, 0.18))
hist(p_hats_j[[1]], main = "p = 0.1, n = 400", xlim = c(0, 0.18))
hist(p_hats_j[[2]], main = "p = 0.02, n = 1040", xlim = c(0, 0.18))
hist(p_hats[[3]], main = "p = 0.02, n = 400", xlim = c(0, 0.18))

```

**As _n_ and _p_ become smaller and we go below the support>=10, the sampling distribution becomes not normal. Our reliance on normal data falls apart for situations that don't meet our inference criteria. At least in this simulation.**
...
**For Australia, yes the conditions for inference are still meet (support=104, not = 936), but Ecuador no (support =8, not = 392)**
...
**So lucky I did Spain! 2012 will be first.**
...
**They are shifted by about 1%, which seems pretty similar, but we could do a difference of two proportions to find out if it is significant.**

    **b.** Is there convincing evidence that the United States has seen a
    change in its atheism index between 2005 and 2012?
    
**H0: There has been no change in atheism index from 2005 to 2012,p_05 - p_12 = 0,  HA: there has been a meaning change in index p_05-p_12 $\neq$ 0**

```{r}
us05 <- subset(atheism, nationality == "United States" & year == "2005")
us05_ath_count<-count(subset(us05, response=="atheist"))
paste("percent atheist in US:",us12_ath_count/dim(us12)[1])
paste("percent atheist in US:",us05_ath_count/dim(us05)[1])
p_05<-us05_ath_count/dim(us05)[1]
p_12<-us12_ath_count/dim(us12)[1]
p_05_est<- us05_ath_count/(us05_ath_count+dim(us05)[1])
p_12_est<- us12_ath_count/(us12_ath_count+dim(us12)[1])
point_est<-p_12_est - p_05_est
p_pooled<- (us05_ath_count + us12_ath_count) / (us05_ath_count + us12_ath_count + dim(us05)[1]+dim(us12)[1])
SE<- sqrt( (p_pooled*(1-p_pooled)/dim(us05)[1]) + (p_pooled*(1-p_pooled)/dim(us12)[1]) )
Z<-as.numeric(point_est/SE)
pnorm(Z, lower.tail = F)*2
```

**Reject H0 since the p-value is << 0.05. There has been a meaningful change.**
...
**About 5% of the time so about 2 countries would we have incorrectly rejected H0**
...
**If we don't have a good number (or any idea) for p, then we will take the worse case scenario of p=0.5**

**$1.96*\sqrt(.25/n)<0.01$, solving this we get that $n=9604$ people would need to be sampled to meet the needed supports.**
