...
**At first, I would look at a scatter plot. This looks reasonably linear, but a more careful look is warranted.**    

```{r}
library(tidyverse)
mlb11 %>%
  select(runs, at_bats) %>%
  ggplot(aes(x=runs, y=at_bats)) +
  geom_point()
```
...
**This data appears to be linear with a positive trend. It has a clear direction though the form may point to issues with constant variance. There is at least one outlier for a large number of runs with few at bats. A correlation coefficient of 0.67 is not bad given the type of data, so it merits a closer look.**
...
**The variables `new_slug` and `new_onbase` are fairly low in SSE compared to `at_bats`. `bat_avg` is also pretty low comparatively.**
...
```{r}
homers<-lm(runs ~homeruns,data=mlb11)
summary(homers)
```
...
```{r}
paste0("runs = ",homers$coefficients[2],'*homeruns + ',homers$coefficients[1])
```

**This is cool, the slope tells us that for every homerun there is nearly a 2 run increase.**
...
```{r}
pred<-m1$coefficients[2]*5578 + m1$coefficients[1]
paste("predicted runs", pred)
mlb11 %>% select(at_bats, runs) %>%filter(at_bats >= 5570) %>% arrange(at_bats) 
```

```{r}
res <- 713 - pred
paste0("closest residual= ", res)
```

**This model would return an overestimate of the numbers of runs to expect given the at bats.**
...
**This residual plots looks pretty good in that it is distributed around zero fairly well. There is an outlier at around 5520 at bats. This plots also seems to have a constant variance.**
...
**depending on how "nearly" we are okay with, this could meet that standard. The histogram seems pretty normal but qqplot is less great. Generally, the nearly normal assumption seems pretty generous, so we can keep it for now.**
...
**I'll choose the old metric of batting average since, I hope, there is some value to it.**

```{r}
lm_bat<- lm(runs ~ bat_avg, data=mlb11)
summary(lm_bat)
```
...
mlb11 %>%
  ...
  geom_abline(slope = lm_bat$coefficients[2], 
              intercept = lm_bat$coefficients[1],
              color="red")
```

**Looks pretty good. A few ood points above and below.**
...
**Batting average is very comparable to at_bats in terms of R squared, residuals, and standard error coefficient ratio. If at_bats is a good predictor then batting average is as well.**
...
```{r}
# regress and print all metrics
# metric_list<-c("hits", "strikeouts", "stolen_bases", "wins","homeruns")
# for (i in metric_list){
#   print(noquote(i))
#   linreg<-lm(runs ~ mlb11[,i], data=mlb11)
#   print(summary(linreg))
# }
# Homeruns was the best, which I already have from above. 
summary(homers)
```
...
mlb11 %>%
  ...
  geom_point(color="black") +
  geom_abline(slope = homers$coefficients[2], 
              intercept = homers$coefficients[1],
              color="red")
```
...
metric_list<-c("new_onbase", "new_slug","new_obs")
for (i in metric_list){
  print(i)
  linreg<-lm(runs ~ mlb11[,i], data=mlb11)
  print(summary(linreg))
...
**The three new metrics; new_onbase, new_slug, new_obs are huge improvements of the previous predictors. R, R squared, residual distribution are all significantly better than the top metrics from the past (homeruns, at bats). So the top predictors are the all three new one plus homeruns and at bats where homeruns and at bats are the worst of those five.** 
...
```{r}
lm_obs<-lm(runs ~ new_obs, data=mlb11)

hist(lm_obs$residuals)
plot(lm_obs$residuals ~ mlb11$new_obs)
abline(h = 0, lty = 3) 
```

**New On Base and Sluging is the top metric, and upon further inquery, it continues to look very good.**
