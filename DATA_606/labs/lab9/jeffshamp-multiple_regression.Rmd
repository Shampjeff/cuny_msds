...
**This is an obsevational study and cannot be generalized to the population but can show associations from this sample set. Thus, it is not a question that can be answered directly from this study alone. A rephrasing might be, Is there an association between evaluation and preceived beauty in some university students?**
...
```{r}
paste("equation score =",m_bty_gen$coefficients[2],"*(beauty score) +",m_bty_gen$coefficients[3]+m_bty_gen$coefficients[1])
```
...
**If I had to guess, I would say something like course credit would the least strong predictor.**
