...
**A length of zero is a single miss after the end of a hit streak.** 
...
**This is a right-skewed distribution with a typcial streak of 0, we see this with the median function
```{r}
median(kobe_streak)
```
...
**Exactly 20 in 100 came up heads.**
...
**We needed to change the size of the sample and add a probability to the sample as well.**
...
```{r}
indy_streak <- calc_streak(sim_basket)
barplot(table(indy_streak))
```
...
**Above we see that simulated streaks of an independent shooter with 45% hit has a distribution similar to Kobe from the NBA finals in 2009, with some important caveats. This distribution has less miss streaks and more frequent non-zero streaks. Also the median streak length is 1, which isn't much higher than Kobe, but is non-zero. In this context, we see that an independent shooter is no different that a supposed hot-hand. We could make a conjecture based on single simulations that an independent shooter would have (potentially) less miss-streak and some longer hit streaks as well.**
...
**I think it would be reasonably similar in the the median streak would not be that much different over the 133 samples. The median streak would likely be between 0 and 1 as a guess. If we ran a simulation with only 20 samples then we could expect very different results. Let's try it again for curiousity sake.** 

```{r}
sim_basket_2<- sample(outcomes, size=133, replace=TRUE, prob = c(.45, .55))
indy_streak_2 <- calc_streak(sim_basket_2)
barplot(table(indy_streak_2))
median(indy_streak_2)
```
...
**Kobe's actual streak distribution has a median of 0 and the number of consecutive shots drops off by 4, where as in both simulations the max streak length is either higher (5 or 7) or has less miss streaks and more hit streaks of all values. That's a big difference in terms of streaks, but there comparable median streak values makes the case for no difference between a hot hand and independent shooter. This suggests that the hot hand theory, does not have much merit. We would like to think our confidence can *will* the the ball in the basket, but I don't there is much behind that**
