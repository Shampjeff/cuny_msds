...
```{r}
library(ggplot2)
ggplot(mdims, aes(x=hgt)) +
  geom_histogram(binwidth = 1.5, position = "dodge", color="red", alpha=0.3) +
  geom_histogram(data = fdims, aes(x=hgt), binwidth = 1.5,
                 position = "dodge", color = "blue", alpha=0.5)+
  labs(x = "height")+
  theme(legend.position = "top")+
  guides(color="legend")
```

**Other than the obvious left/right shift these two histograms look about the same in terms of shape. Maybe there is a binomial look to the female histogram, but they both are similar in terms of nearly normal.**
...
**It's pretty close! The peak of the bars is a bit below the denisty plot peak and some of the bars are either too wide or short, but it's pretty good.**
...
**The data produced from a definitely normal sample looks more stable throught the -1.5 to 1.5 Z-score range, but the tails are just as errant. Also it is clear that not _all_ the points fall on the line.**
...
**This appears to be the case. The simulated data looks pretty similar to the actual data, albiet a bit smooother.**

5.  Using the same technique, determine whether or not female weights appear to 
    come from a normal distribution.
    
**Let's look at the QQ plot for weight in detail, then do the simulation.**
...
**So the weight data is more skew than height. The "belly flop" over the fixed line suggests a right skew to this data.**

```{r}
qqnormsim(fdims$wgt)
```

**Again, this looks a bit different than the simulated data in that it is more curved and makes slight, shallow U shape. Does this mean it is not normal enough? That is unclear, but if things start to look strange after we assume normality, then we have a theory as to why.**
...
**Start with heights. _What is the probability that a randomly selected female will be less than 5'5" (160cm)?_**

**Theory**

```{r}
h_1<-pnorm(160, mean = mean(fdims$hgt), sd = sd(fdims$hgt))
h_1
```
...
```{r}
h_2<-sum(fdims$hgt < 160) / length(fdims$hgt)
h_2
```

**Now let's look at a simple percent difference**
...
**Weights. _What is the probability that a randomly selected female will weigh more than than 50Kg (110lbs)?_**

**Theory**

```{r}
w_1<-1-pnorm(50, mean = mean(fdims$wgt), sd = sd(fdims$wgt))
w_1
```
...
```{r}
w_2<-sum(fdims$wgt > 50) / length(fdims$wgt)
w_2
```
...
**The values with the weight seemed more closely aligned. Did not expect that. Does this mean the weight data with a right skew is more normal (or better approximates normal) than the more straight but shaky height data?** 
...
-   Now let's consider some of the other variables in the body dimensions data 
    ...
    **a.** The histogram for female biiliac (pelvic) diameter (`bii.di`) belongs
    to normal probability plot letter _**B**_.


    **b.** The histogram for female elbow diameter (`elb.di`) belongs to normal 
    probability plot letter _**C**_.
    ...
    **d.** The histogram for female chest depth (`che.de`) belongs to normal 
    probability plot letter _**A**_.
    
-   Note that normal probability plots C and D have a slight stepwise pattern.  
    Why do you think this is the case?

**C and D correspond to age and elbow diameter. Age makes sense in that is it discrete (no one says they are 51.5 years old). Elbow diameter is odd, but they might be some reason why the values seem to pile up at (close to) discrete intervals.** 
...
**This looks to be fairly normally distributed with a slight right-skew. There are some "extra" values in the 2-3 Z-score range.**

```{r}
hist(fdims$kne.di)
```

**Makes sense given the histogram**
