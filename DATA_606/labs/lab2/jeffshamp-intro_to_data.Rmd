...
**BMI as a function of height**
```{r}
boxplot(bmi ~ cdc$height,xlab = 'Height' )
```

**The result here get a little strange to understand on the very high and low range of heights, but appear consistent for many height values. This could be easier to see if we could bin the height values more. Something like 20-25, 26-30, etc.**
...
```{r}
plot(cdc$wtdesire ~cdc$weight, xlab= 'weight', ylab = 'desired weight')
```
**This is a plot of peoples desired weight as a function of actual weight. A line with a slope of 1 would represent that everyone is happy with their current weight. It seems that most poeple have a desired weight that is below their current weight with some exceptions especially in the under 200 range.** 
...
summary(cdc$wdiff)
```
**Most people want to lose somewhere between 10 and 15 lbs**
...
**The way I have set it up, `wdiff` is a vector in the data matrix or column in the data frame. If the observation is 0, then the person would like no change in weight, negative values represent a desired loss in weight and positive values represent desired gain.**
...
**We saw in the summary above that the IQR is relatively small, around 6.5 lbs, but the min and max values are huge by comparison. So most people seem to want a small change in weight and a significant number of people who want to lose between 20 and 200 lbs. A small minority seem to have unrealistic ideas about weight (like the person who wants to gain 500lbs).**
...
**The boxplot tells a similar story, a narrow range of desiring a small loss in weight with many values outside of the quartiles.**

-   Using numerical summaries and a side-by-side box plot, determine if men tend
    to view their weight differently than women.
    
```{r}
boxplot(cdc$wdiff ~ cdc$gender, xlab='Gender', ylab='desired weight difference')
summary(subset(cdc, subset=gender=='m', select = wdiff))
```
**The boxplots don't offer too much more information, but the summary tables are interesting. Above is the summary for men**

```{r}
summary(subset(cdc, subset=gender=='f', select = wdiff))
```

**Here is the summary for women. We can see that women and men are very similar in terms of desired weight loss. The difference from first quartile to median are both about 15 lbs, but women have a large IQR of 27 lbs compared to 20lbs for men.**
...
```{r}
wt_std<-sd(cdc$weight)
wt_mean<-mean(cdc$weight)
out_mean<- subset(cdc, weight > (wt_mean - wt_std) & weight < (wt_mean + wt_std))
dim(out_mean)[1] / dim(cdc)[1]
```
**This makes sense since with this much data we should expect that around 67% of the data falls within a two standard deviation range (one std on each side of the mean). **
