...
**This appears to be normally distributed, if a bit right skewed, data with a mean of 1500 sqm. There is a right tail to this data, observations above 3000 sqm appear to be outliers as shown in the box plot.**
...
**This is much less normal-looking. It's hard to tell what, if any, distribution there is.**
...
**The law of high numbers tells us that as the sample size increases, the sample will look more like the population. For the size 100 sample, the distribution doesn't look that much different but does seem to be a step in the right direction.**
...
**This effect is nothing short of amazing. From what appears to incoherance we find order, amazing.**
**The mean of the sample distribtion (center) is 1500.7, unbelievably close to the true population mean and is a normal distribuution.**
...
sample_means_small[80:100]
```

**This is a vector of the mean value of each 50 sample set of areas fro Ames, Iowa.**
...
**The center of sample distributions stays remarkably constant around 1500 sqm (the true population mean), but the spread is higher for smaller sample sizes.**
...
**A simple mean of the sample is our point estimate.**
...
paste("Mean of the sampling distribution sized 50: ",
      mean(sample_means50))
paste("Mean of the population for SalesPrice: ", 
      mean(price))
hist(sample_means50, 25)
```
...
hist(sample_means50,25, xlim=xlimits)
hist(sample_means150, 25, xlim=xlimits)
paste("Mean of the 150 sized sampling distribution: ", 
...
**The sample distribution with from a sample size of 150 has the smaller spread. We want our estimates to be tightly bound to be meaningful, so the smaller spread is better. The wider spread seems to be the result of capturing more tailend points.**
