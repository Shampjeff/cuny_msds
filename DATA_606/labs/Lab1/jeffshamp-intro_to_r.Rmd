...
**It is increasing for the most part year-over-year. There is some issue between 1640 and 1665, where in the number of boys and girls drops dramatically, but other than that there appears to be a linear relationship to the number of births as a function of time.** 
...
```{r}
plot(x=arbuthnot$year, y= (arbuthnot$boys/(arbuthnot$girls + arbuthnot$boys)), 
     xlab = "Year", ylab = "Proportion of Boys Born")
```
...
**In three columns and 63 rows with have a dataframe of the birth population of boys and girls for a given year from 1940 - 2002. The summary command gives us a nice layout of the numbers. **
    

-   How do these counts compare to Arbuthnot's? Are they on a similar scale?

```{r}
plot(present$year, present$boys,xlab= "year", ylab = "population of boys")
plot(present$year, present$girls,xlab= "year", ylab = "population of girls")
```

**These numbers are several orders of magnitude higher than Arbuthnot's sample.It is similar in that it is increasing, but we would need to examine the derivative to understand the relative rates of growth. This population has both a boom and and a dip in the population pattern.**
...
plot(present$year, (present$boys/(present$boys+present$girls)),
     xlab= "year", ylab = "Ratio of Boys to Girls")
```


**It also shows a ratio that is greater than 50% for births of boys, which is inline with Arbuthnot's findings. I am actually a bit shocked by this, I would have expected at least a few years of less boys than girls.**

**The spread of values for this data does seem to be tighter than Arbuthnot's data. There is a 3% change from max to min birth ratio in Arbuthnot's data and closer to a 1.5% max to min spread in this data. It also apprears to be getting more even, which is strange. Prehaps this is an example of the law of high numbers? The U.S. population is much higher than London 1600s, and as such the ratio is more even? Or prehaps there is a boy bias that has diminished?**
...
```{r}
subset(present, boys==max(boys) & year)
```

**1961 - Right at the tail end of the baby boom.**

```{r}
present$total<- present$boys + present$girls
subset(present, total==max(total) & year)
```

**This is consistent with total maximum birth year (sum of boys and girls).**
